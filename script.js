...
// Initialize lookAtTarget
const lookAtTarget = new THREE.Object3D();
scene.add(lookAtTarget); // Add it to the scene instead of the camera

/**
 * Updates the position of lookAtTarget based on provided X and Y coordinates.
 * @param {number} x - The X coordinate.
 * @param {number} y
 * @param {number} z - The Y coordinate.
 */
// Assuming camera_resolution_width and camera_resolution_height are the dimensions of your input source
const camera_resolution_width = 1280;  // Replace with actual width
const camera_resolution_height = 720; // Replace with actual height

function updateLookAtTarget(x, y, z) {
  // Normalize or scale x and y if needed
  // For example, you might want to map these from a range of [-1, 1] or based on your scene's scale
  let normalizedX = x; // Adjust this based on your needs
  let normalizedY = y; // Adjust this based on your needs
  let normalizedZ = z;

  // Update the lookAtTarget position
  lookAtTarget.position.set(normalizedX, normalizedY, normalizedZ); // Assuming Z remains constant
}

// Example usage
updateLookAtTarget(0, 1, 1); // Update this with actual X, Y values as needed
...
currentVrm.blendShapeProxy.setValue(THREE.VRMSchema.BlendShapePresetName.Angry, expressionoof);
    ...
    // lookAtTarget.position.set(fixedX, fixedY, fixedZ);
...
function blink() {
  // Calculate blink timeout (if it's used for triggering blinks)
  var blinktimeout = Math.floor(Math.random() * 250) + 50;

  // Update VRM lookAt target and reset bone manager
  // This now points to the static lookAtTarget
